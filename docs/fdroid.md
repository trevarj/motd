...
full source checkout and is signed by F-Droid; the GitHub release APK is not a
build input.
...
AAR in the build directory from three pinned source libraries:

```yaml
rm: app/libs/libbox.aar,app/src/google,firebase
ndk: 28.0.13004108
srclibs: SingBox@1086ab2563320e0da0c23b3a491d8dfa0939dff4,SingBoxAndroid@772879ce9cd37c29e377d4d44d0efee12662948d,GoMobile@b2c30f47825831593d6980af8191527490f9c968
build:
  - export LIBBOX_SOURCE_DIR="$$SingBox$$"
  - export LIBBOX_ANDROID_SOURCE_DIR="$$SingBoxAndroid$$"
  - export GOMOBILE_SOURCE_DIR="$$GoMobile$$"
  - export LIBBOX_OUTPUT_DIR="$PWD/app/build/generated/libbox"
...
The three source libraries need matching files under `srclibs/` in the
...
The source builder verifies each checkout's commit and archive hash, validates
NDK `28.0.13004108`, and rejects any JNI entry other than
...
metadata and its three `srclibs` files:
