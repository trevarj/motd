...
class SpacingTest {
    ...
    @Test
    fun cozy_tokens() {
        val s = spacingFor(LayoutDensity.COZY)
        assertEquals(2.dp, s.bubbleRowVPad)
        assertEquals(8.dp, s.bubbleInnerVPad)
        assertEquals(12.dp, s.bubbleInnerHPad)
        assertEquals(20.dp, s.bubbleCorner)
        assertEquals(36.dp, s.bubbleAvatar)
        assertEquals(44.dp, s.bubbleAvatarColumn)
        assertEquals(4.dp, s.actionVPad)
        assertEquals(6.dp, s.systemPillVPad)
